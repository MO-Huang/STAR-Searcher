...
const int BsplineOptimizer::YAWFEASIBILITY = (1 << 9);
...
void BsplineOptimizer::setParam(ros::NodeHandle& nh) {
  ...
  nh.param("optimization/max_yaw_change", max_yaw_change, -1.0);
...
void BsplineOptimizer::setCostFunction(const int& cost_code) {
  ...
  if (cost_function_ & YAWFEASIBILITY) cost_str += " yaw_fea  |";
                                            ...
                                            double &cost, vector<Eigen::Vector3d> &gradient_q)
{
  ...
  for(int i = 0; i < q.size(); ++i)
  {
    // control point of vel
    Eigen::Vector3d vi = (q[i + 1] - q[i]) * dt_inv;
    double vd = fabs(vi[0]) - max_yaw_change;
    if(vd>0.0)
    {
      cost += pow(vd,2);
