...
void SDFMap::initMap(ros::NodeHandle &nh) {
  ...
  nh.param("sdf_map/belief_dist", mp_->belief_dist_, 6.0);
  ...
  for (int i = 0; i < 3; ++i) {
    nh.param("sdf_map/box_min_" + axis[i], mp_->box_mind_[i],
             mp_->map_min_boundary_[i]);
    nh.param("sdf_map/box_max_" + axis[i], mp_->box_maxd_[i],
             mp_->map_max_boundary_[i]);
  }
...
void SDFMap::resetBuffer(const Eigen::Vector3d &min_pos,
...
void SDFMap::inputCamLidarPointCloud(const Eigen::MatrixXd &points,
...
void SDFMap::clearAndInflateLocalMap() {
  ...
  // Eigen::Vector3i min_cut_m = min_cut - Eigen::Vector3i(vec_margin,
  // vec_margin, vec_margin); Eigen::Vector3i max_cut_m = max_cut +
  // Eigen::Vector3i(vec_margin, vec_margin, vec_margin); boundIndex(min_cut_m);
  ...
  //       mp_->unknown_flag_; md_->distance_buffer_all_[idx] = 10000;
  //     }
...
double SDFMap::getDistWithGrad(const Eigen::Vector3d &pos,
                               Eigen::Vector3d &grad) {
  if (!isInMap(pos)) {
    grad.setZero();
    return 0;
...
double SDFMap::getZ() { return mp_->map_size_(2); }

double SDFMap::getBeliefDist() { return mp_->belief_dist_; }

void SDFMap::getDistanceAndGradient(const Eigen::Vector3d &pos,
                                    double &distance,
  ...
  gradient[2] = (v1 - v0) * mp_->resolution_inv_;
  ...
  gradient[0] *= mp_->resolution_inv_;
}

void SDFMap::getDistanceAndGradient(const Eigen::Vector3i &idx,
                                    double &distance,
                                    Eigen::Vector3d &gradient) {
  // CHECK(isInMap(idx));
  Eigen::Vector3d pos;
  indexToPos(idx, pos);
  getDistanceAndGradient(pos, distance, gradient);
}
} // namespace fast_planner
// SDFMap
