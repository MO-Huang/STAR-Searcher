...
public:
  ...
  void planExploreTraj(const vector<Eigen::Vector3d> &tour,
                       ...
                       const double &time_lb = -1);
  int planLocalMotion(const Vector3d &next_pos, const Vector3d &pos,
  ...
  // void planYawInfo(const Eigen::Vector3d &start_yaw, const double &end_yaw,
  //                  const bool &end_yaw_specified, const double &relax_time);
...
private:
  ...
  Eigen::MatrixXd reparamLocalTraj(const double &start_t,
...
public:
  bool localExplore(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
